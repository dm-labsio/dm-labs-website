...
describe("Hebrew locale foundation", () => {
  ...
  it("exposes only reviewed Hebrew routes and no invented child routes", () => {
    ...
    expect(router).not.toMatch(/path="\/he\/(?:faq|blog|terms|privacy|cookies)/);
    expect(prerender).not.toMatch(/"\/he\/(?:faq|blog|terms|privacy|cookies)/);
    expect(serverRoutes).not.toMatch(/"\/he\/(?:faq|blog|terms|privacy|cookies)/);
  });
});
