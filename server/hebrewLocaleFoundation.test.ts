...
describe("Hebrew locale foundation", () => {
  ...
  it("exposes only reviewed Hebrew routes and no invented child routes", () => {
    ...
    expect(router).not.toMatch(/path="\/he\/(?:blog|terms|privacy|cookies)/);
    expect(prerender).not.toMatch(/"\/he\/(?:blog|terms|privacy|cookies)/);
    expect(serverRoutes).not.toMatch(/"\/he\/(?:blog|terms|privacy|cookies)/);
  });
});
