...
export default function WebDesignCyprus() {
  ...
  return (
    <main className="bg-[#F6F6F4] min-w-0 overflow-x-hidden">
      ...
      <section className="section-spacing bg-white">
        <div className="container max-w-5xl mx-auto"><AnimateIn><h2 className="text-3xl font-bold text-[#111315] mb-4 text-center">Website Packages</h2><p className="text-[#5B6472] text-center mb-10">Clear packages for common website needs, with a custom quote for work beyond the standard scope.</p></AnimateIn><div className="grid grid-cols-1 md:grid-cols-3 gap-6">{packages.map((plan, index) => <AnimateIn key={plan.name} delay={index * 0.1}><div className={index === 1 ? "brand-gradient-border h-full" : "h-full"}><div className="dm-card h-full !shadow-none flex flex-col"><p className="text-sm font-semibold uppercase tracking-wide text-[#5B8CFF] mb-2">{plan.name}</p><p className="text-3xl font-bold text-[#111315] mb-4">{plan.price}</p><p className="text-sm text-[#5B6472] leading-relaxed flex-1">{plan.details}</p><StarButton asChild><Link href="/contact/" className={index === 1 ? "btn-primary w-full justify-center mt-6" : "btn-secondary w-full justify-center mt-6"}>Discuss This Package</Link></StarButton></div></div></AnimateIn>)}</div><AnimateIn delay={0.35}><div className="mt-8 bg-[#0F172A] rounded-2xl p-7 text-white flex flex-col md:flex-row gap-6 md:items-center md:justify-between"><div><p className="text-sm font-semibold text-[#6FE3FF] uppercase tracking-wide">Enterprise / Custom</p><h3 className="text-2xl font-bold mt-1 enterprise-scope-title">Pricing tailored to your scope</h3><p className="text-sm text-white/70 mt-2 max-w-2xl">For integrations, multilingual websites, CMS self-editing, AI or chatbot features, complex motion, CRM or booking, and unusual content volume.</p></div><StarButton asChild><Link href="/contact/" className="btn-primary shrink-0">Request a Quote</Link></StarButton></div></AnimateIn></div>
      </section>
